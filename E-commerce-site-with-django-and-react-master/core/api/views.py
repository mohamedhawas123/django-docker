...
from .serializers import (
    ItemSerializer, OrderSerializer, ItemDetailSerializer, AddressSerializer, PaymentSerializer
)
...
class AddToCartView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        if len(variations) < minimum_variation_count:
            ...
            return Response({"message: Not right"}, status=HTTP_400_BAD_REQUEST)
...
class PaymentView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        selected_billing_id = request.data.get('selectedBillingadress')
        selected_shipping_id = request.data.get('selectedShipingadress')
...
class AddressListView(ListAPIView):
    ...
    def get_queryset(self):
        address_type = self.request.query_params.get('addressType', None)
...
class CreateAddressListView(CreateAPIView):
    permission_classes = (IsAuthenticated, )
    serializer_class = AddressSerializer
    queryset = Address.objects.all()


class UpdateAddressListView(UpdateAPIView):
    permission_classes = (IsAuthenticated, )
    serializer_class = AddressSerializer
    queryset = Address.objects.all()
